...
try:
    from hungyi_chinese import expand_query_tokens, is_stop_token, s2t
except ImportError:
    from scripts.hungyi_chinese import expand_query_tokens, is_stop_token, s2t
...
def query_graph(G: nx.Graph, partition: dict[str, int], query: str, budget: int = 20) -> dict:
    ...
    raw_tokens = [t.lower().strip() for t in re.split(r"[\s,;:()\"'`/]+", query) if t.strip()]
    content_tokens = [t for t in raw_tokens if len(t) >= 2 and not is_stop_token(t)]
    if not content_tokens:
        content_tokens = [t for t in raw_tokens if len(t) >= 2]
    expanded = expand_query_tokens(content_tokens)
    tokens = [t for t in expanded if not is_stop_token(t)] or expanded
        ...
        for part in zh_parts:
            if len(part) > 2:
                for i in range(len(part) - 1):
                    sub = part[i:i+2]
                    if sub not in tokens and sub not in extra_tokens and not is_stop_token(sub):
    ...
    full_query_trad = s2t(full_query)
    ...
    for nid, data in G.nodes(data=True):
        ...
        if (full_query and full_query in label) or (full_query_trad and full_query_trad in label):
            score += 5
...
def export_html(
    ...
    community_legend = json.dumps(
        {str(k): html.escape(str(v)) for k, v in community_labels.items()}, ensure_ascii=False
    )
...
def build_full_graph(
    ...
    print("  GRAPH_REPORT.md  — god nodes, surprising connections, suggested questions")
