...
# Add scripts directory to sys.path to allow clean imports of helper modules
_SCRIPTS_DIR = Path(__file__).resolve().parent
if str(_SCRIPTS_DIR) not in sys.path:
    sys.path.insert(0, str(_SCRIPTS_DIR))

try:
    from hungyi_chinese import STOP_QUERY_TOKENS, expand_query_tokens, is_stop_token
except ImportError:
    from scripts.hungyi_chinese import (
        STOP_QUERY_TOKENS,
        expand_query_tokens,
        is_stop_token,
    )
...
# Re-export STOP_QUERY_TOKENS for callers/tests accessing kb.STOP_QUERY_TOKENS
STOP_QUERY_TOKENS = frozenset(STOP_QUERY_TOKENS)
    ...
    if graph_report.exists() or graph_json.exists():
        ...
        if graph_json.exists():
            try:
                ...
                print(f"[debug] Notice: unable to read graph stats from {graph_json}: {err}")
        index_text += f"""
...
def tokenize_query(query: str) -> list[str]:
    ...
    for token in raw:
        token = token.strip()
        if not token or is_stop_token(token):
            continue
        tokens.append(token)
    expanded = expand_query_tokens(tokens)
    filtered = [t for t in expanded if not is_stop_token(t)]
...
def run_graph_build() -> None:
    ...
    from hungyi_graph import build_full_graph
...
def run_graph_report() -> None:
    ...
    from hungyi_graph import generate_report, label_communities
    from networkx.readwrite import json_graph as nx_json
