...
本 repo fork 自 [`voidful/hung-yi-lee-skill`](https://github.com/voidful/hung-yi-lee-skill)，
沿用 MIT License 與完整 Git 歷史。

## 為什麼維護 fork

- 保留原作者持續更新的李宏毅老師 AI/機器學習課程知識庫、逐字稿快取、知識圖譜與 Agent Skill。
...
| `AGENTS.md` / `CLAUDE.md` / `GEMINI.md` | 本 fork 的 AI 維護單一真相源（兼載 Wiki 結構規範） |
...
| `tools/test_product.ps1` | Windows 原生產品測試執行腳本（驗證 lint、search 與 graph query） |
| `requirements.txt` / `requirements-dev.txt` | 產品與維護依賴清單 |
...
核心腳本在 `scripts/`、知識庫在 `wiki/` 與 `raw/`，以上游為準。

## 分支與 remote

- `origin/main`：SanHsien 維護線，也是唯一長期分支。
- 日常修改在本機跑 gate 後直接推 `origin/main`。
- `upstream/main`：voidful 原始專案，只追蹤、不推送。
...
# git remote add upstream https://github.com/voidful/hung-yi-lee-skill.git
pwsh -NoProfile -File tools\bootstrap_dev.ps1
```

要實際使用知識庫 CLI 與圖譜導航，見 [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md)。
