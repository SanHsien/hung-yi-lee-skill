...
def test_readme_pair_cross_links_and_names_the_fork() -> None:
    ...
    assert "voidful/hung-yi-lee-skill" in zh
    assert "voidful/hung-yi-lee-skill" in en
...
def test_baseline_file_is_valid_and_complete() -> None:
    ...
    assert baseline["repo"] == "https://github.com/voidful/hung-yi-lee-skill.git"
    assert baseline["branch"] == "main"
    assert len(baseline["reviewed_through"]) == 40
    assert baseline["reviewed_through"] == "df5665d921a94c7937c27cf3c32e582008a95652"
...
def test_baseline_matches_decisions_record() -> None:
    ...
    assert "df5665d" in upstream
    assert "voidful/hung-yi-lee-skill" in decisions
